...
public class AddUserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// Getting attribute values through the request and using set methods to
		...
		IuserServices iuserservices = new UserServices();// Creating interface
															// object and
															// assigning it with
															// new userServices
															// object
		...
		if (!(user.getName().isEmpty() || user.getEmail().isEmpty() || user.getNic().isEmpty()
				|| user.getPassword().isEmpty())) {
			if (!(iuserservices.CheckEmailExists(user))) {
				iuserservices.RegisterUser(user); // Executing RegisterUser
		...
		RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/index.jsp"); // Include
																								// returning
																								// view
